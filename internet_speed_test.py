...
duration = 600 #? {seconds} How long do you want to test for?
...
while time_now <= target_time: # While we still have time to run:
    ...
    df.loc[len(df.index)] = [
        ...
        down,
        up
    ] # Creat a new entry containing the time and results.
    time_now = datetime.datetime.now() # Check the time.

print(df) # Print the results to the terminal.

df.to_excel(f"results{time_now.year}-{time_now.month}-{time_now.day}-{time_now.hour}h{time_now.second}s.xlsx") # Export the results to a spreadsheet.